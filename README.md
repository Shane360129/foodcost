...
- **食材主檔 Ingredient master** — 名稱、單位（公克／毫升／個／份）、單位成本、供應商備註；內建 **60+ 常見食材**（牛腱、雞胸、洋蔥、橄欖油、番紅花…）可一鍵載入。
...
4. **給合夥人／主廚的報表** — 一鍵匯出 A4 PDF，數字攤開來談，不再各說各話。
...
```
src/
├─ types.ts                 # 領域型別（Ingredient / MenuItem / MenuCost…）
├─ lib/
│  ├─ db.ts                 # Dexie 資料庫 + CRUD
│  ├─ calc.ts               # 成本／成本率／毛利／回推售價（純函式）
│  ├─ seed.ts               # 60+ 食材 + 3 道範例菜單
